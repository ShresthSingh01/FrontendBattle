...
export default function NeuralEngine() {
  ...
  return (
    <section id="neural-engine" className="bg-[#0a0a0a] pt-32 pb-24 border-t border-[rgba(255,255,255,0.08)]">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-4 gap-8">
        <div className="col-start-2 col-span-2">
          ...
          <div className="grid grid-cols-4 gap-8">
            {[
              { label: 'Secure Enclave', icon: '🔒' },
              { label: 'Data Connectors', icon: '🔌' },
              { label: 'Compute Nodes', icon: '⚡' },
              { label: 'Memory Stack', icon: '🥞' },
            ].map(item => (
              <div key={item.label} className="text-center">
                <div className="w-16 h-16 mx-auto mb-4 border border-[rgba(255,255,255,0.2)] rounded-lg flex items-center justify-center text-2xl text-white">
                  {item.icon}
                </div>
